...
import com.TresAoCubo.entitys.Bullet;
import com.TresAoCubo.entitys.Enemy;
import com.TresAoCubo.entitys.Entity;
import com.TresAoCubo.entitys.Lifepack;
import com.TresAoCubo.entitys.Weapon;
		...
		try {
			...
			for(int xx = 0; xx < map.getWidth(); xx++) {
				for(int yy = 0; yy < map.getHeight(); yy++) {
					...
					} else if(pixel == 0xFF00137F) {
						// location of player
						Game.player.setX(xx*16);
						Game.player.setY(yy*16);
						
					}else if(pixel == 0xFFB6FF00) {
						//enemy
						Enemy enemy = new Enemy(xx*16, yy*16, 16, 16, Entity.ENEMY_EN);
						Game.entities.add(enemy);
						Game.enemies.add(enemy);
						
					}else if(pixel == 0xFFFF30F1) {
						//weapon
